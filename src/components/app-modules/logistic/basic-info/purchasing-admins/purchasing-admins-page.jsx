...
import PurchasingAdminModal from "./purchasing-admin-modal";
import DetailsModal from "./purchasing-admin-details-modal";
...
const getSheets = (records) => [
  {
    title: "PurchasingAdmins",
    data: records,
    columns: [
      { label: Words.id, value: "AdminID" },
...
const baseColumns = [
  {
    ...
    align: "center",
    dataIndex: "AdminID",
    sorter: getSorter("AdminID"),
    render: (AdminID) => <Text>{utils.farsiNum(`${AdminID}`)}</Text>,
  },
  {
...
const recordID = "AdminID";

const PurchasingAdminsPage = ({ pageName }) => {
  const {
    progress,
    searched,
    searchText,
      ...
      <Spin spinning={progress}>
        <Row gutter={[10, 15]}>
          <SimpleDataPageHeader
            title={Words.purchasing_admins}
            searchText={searchText}
            sheets={getSheets(records)}
            fileName="PurchasingAdmins"
...
export default PurchasingAdminsPage;
