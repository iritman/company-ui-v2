...
const PaymentReceiptChequeModal = ({
  isOpen,
  selectedObject,
  onOk,
  onCancel,
  ...
  useMount(async () => {
    ...
    try {
      ...
      if (selectedObject) {
        ...
        let cheques = await service.getWhiteCompanyCheques(
          selectedObject.CompanyBankAccountID
        );

        // const { CompanyChequeID, ChequeNo } = selectedObject;
        // cheques = [{ CompanyChequeID, ChequeNo }, ...cheques];

        setCompanyCheques(cheques);
  ...
  const handleChangeCompanyBankAccount = async (value) => {
    const rec = { ...record };
    rec.CompanyBankAccountID = value;
    ...
    if (value > 0) {
      try {
        const data = await service.getWhiteCompanyCheques(value);

        setCompanyCheques(data);
      } catch (ex) {
        handleError(ex);
      }
    }
        ...
        <Row gutter={[5, 1]} style={{ marginLeft: 1 }}>
          ...
          <Col xs={24} md={12}>
            <DropdownItem
              ...
              formConfig={formConfig}
              required
              onChange={handleChangeCompanyBankAccount}
            />
          </Col>
          <Col xs={24} md={12}>
            <DropdownItem
              title={Words.cheque_no}
              dataSource={companyCheques}
              keyColumn="CompanyChequeID"
              valueColumn="ChequeNo"
              formConfig={formConfig}
            />
          </Col>
          <Col xs={24} md={12}>
...
export default PaymentReceiptChequeModal;
